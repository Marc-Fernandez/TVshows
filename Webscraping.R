...
for(tconst in Series$tconst){
  ...
  a <- ifelse(Binaria == "Amazon Prime Video" | Binaria == "Netflix" | Binaria == "HBO", 1, 0) # Creamos un vector de tantos valores como distribuidoras la serie tenga.
  a <- ifelse(sum(a) > 0, 1, 0) # resumimos el vector anterior en un vector de una observación.
